...
**Сторонний прокси/шлюз.** Можно работать не с официальным API, а через Anthropic-совместимый прокси (протокол `anthropic-messages`, эндпоинт `/v1/messages`). Задайте его адрес в `.env`:

```
ANTHROPIC_API_KEY=sk-ваш-ключ-провайдера
ANTHROPIC_BASE_URL=https://ваш-прокси.example.com
CLAUDE_MODEL_MAIN=имя-модели-из-каталога-провайдера
```

Требования к прокси, без которых бот потеряет функциональность:
- **tool calling** — обязателен (модель должна возвращать блоки `tool_use` и `stop_reason=tool_use`), иначе не будет доступа к инструментам сервера;
- **vision** — для анализа скриншотов;
- поддержка `tool_result` в последующих сообщениях — для цикла «инструмент → результат → ответ».

⚠️ Через прокси проходят **все** данные диалога, включая логи и конфиги вашего VPS. Используйте `https://`, не `http://`, и доверяйте провайдеру. При `http://`-адресе бот залогирует предупреждение.
